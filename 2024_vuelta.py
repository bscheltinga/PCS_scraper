...
df = pd.read_excel(r'data/2024_08_16_Vuelta_start_list.xlsx')
df_scrape = df.copy()
df_scrape['views'] = np.zeros(len(df))

for i in range(len(df)):
    url = df['URL'][i] + str('/statistics')
...
df_scrape.to_excel('data/2024_08_16_vuelta_scraped_28_days.xlsx', index=False)